...
from . Utilities import (create_materials_palette, assign_color, rgb_to_rgbaf, apply_modifiers, find_collection, make_collection, collectionNavigtor)
...
class MORPHOBLEND_OT_WorkListAdd(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # If the "Work List" collection does not exist create it
        if "Work List" not in bpy.data.collections:
            wl_coll = bpy.data.collections.new(name="Work List")
            bpy.context.scene.collection.children.link(wl_coll)
        else:
            wl_coll = bpy.data.collections["Work List"]
        for obj in bpy.context.selected_objects:
            # Make a COPY (SymLink) of the object to the Work List if it is not already there
            if  bpy.data.collections['Work List'] not in obj.users_collection:
                wl_coll.objects.link(obj)
                info_mess = obj.name + " added!"
                self.report({'INFO'}, info_mess)
            else:
                info_mess = obj.name + " is already in the Work List!"
...
class MORPHOBLEND_OT_WorkListRemove(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        if "Work List" in bpy.data.collections:
            return context.active_object is not None and context.object.select_get() and context.object.type == 'MESH'
    ...
    def execute(self, context):
        ...
        process_op  = scene.process_tool
        for obj in bpy.context.selected_objects:
            if  bpy.data.collections['Work List'] in obj.users_collection:
                # Remove the COPY (SymLink) of the object from the Work List
                wl_coll = bpy.data.collections["Work List"]
                wl_coll.objects.unlink(obj)
                info_mess = obj.name + " removed!"
                self.report({'INFO'}, info_mess)
            else:
                info_mess = obj.name + " is not in the Work List!"
                self.report({'WARNING'}, info_mess)
        return{'FINISHED'}
...
class MORPHOBLEND_OT_WorkListNext(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        if "Work List" in bpy.data.collections:
            obj_in_wp  = bpy.data.collections['Work List'].objects[:]
            if len(obj_in_wp) > 0:
                return True
    ...
    def execute(self, context):
        ...
        if len(bpy.context.selected_objects) == 1:
            ...
            obj = bpy.data.collections['Work List'].objects[0]
        else:
            obj = bpy.data.collections['Work List'].objects[0]
            self.report({'WARNING'}, "Only one cell can be selected")
        result = collectionNavigtor("Work List", obj, "next")
...
class MORPHOBLEND_OT_WorkListPrevious(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(bpy.context.selected_objects) == 1:
            ...
            obj = bpy.data.collections['Work List'].objects[0]
            self.report({'WARNING'}, "Only one cell can be selected")
        result = collectionNavigtor("Work List", obj, "previous")
        if result is not False:
            # deselect current obj and make the next one active & selected
            bpy.context.active_object.select_set(state=False)
            bpy.context.view_layer.objects.active = result
...
class MORPHOBLEND_OT_Split_finish(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        split_object.name = bpy.context.scene.objects[original_object_name].name + "_split"
...
class MORPHOBLEND_PT_Edit(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if "Work List" in bpy.data.collections:
            obj_in_wp  = bpy.data.collections['Work List'].objects[:]
            if len(obj_in_wp) > 1:
                text_box = "Work List (" + str(len(obj_in_wp)) +" cells)" + " - [" + str(len(bpy.context.selected_objects)) + " selected cell(s)]"
            else:
                text_box = "Work List (" + str(len(obj_in_wp)) +" cell)" + " - [" + str(len(bpy.context.selected_objects)) + " selected cell(s)]"
        else:
            text_box = "Work List (0 cell)" + " - [" + str(len(bpy.context.selected_objects)) + " selected cell(s)]"
        ...
        row = box.row()
        if len(bpy.context.selected_objects) > 1:
            text_box = "Merge/Split [" + str(len(bpy.context.selected_objects)) + " selected cells]"
        else:
            text_box = "Merge/Split [" + str(len(bpy.context.selected_objects)) + " selected cell]"
